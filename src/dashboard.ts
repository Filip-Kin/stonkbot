    ...
    *{box-sizing:border-box;}
    ...
    <div class="topbar">
      ...
      <div style="display:flex;gap:.6rem;align-items:center;flex-wrap:wrap">
        <a class="pill" href="/arena.pdf" style="color:var(--violet);border-color:rgba(169,123,255,.4)">⚔️ Arena results (PDF)</a>
        ${marketStatus}
      </div>
    </div>
...
const server = Bun.serve({
  ...
  async fetch(req) {
    ...
    // The Arena ended 2026-09-11. Its live pages are gone; what survives is the
    // written analysis, served as a file so it needs no database and no Alpaca
    // call. Generated by arena-analysis/make-pdf.py.
    if (pathname === "/arena.pdf") {
      const pdf = Bun.file(new URL("../public/arena-summary.pdf", import.meta.url).pathname);
      if (!(await pdf.exists())) return new Response("not found", { status: 404 });
      return new Response(pdf, {
        headers: {
          "content-type": "application/pdf",
          "content-disposition": 'inline; filename="stonkbot-arena-results.pdf"',
